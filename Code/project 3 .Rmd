...
So, we transform our response variable to the 1/4 order. The residual plot shows a little quadratic trend , thus we add the polynomial terms to our model.
...
The summary output of our final model is shown as follows. From the output, we can see that diabetes,Obesity and DevelopedCTYES are significant at level 0.01, which means that there are association between our response variable COVID-19 diagnostics rate and these significant variables. 
Furthermore, we make two plots to show the relationship  of” y  vs obesity “ and “y vs diabetes”, note that we has already include their second-order term, however, they still show a linear trend because the coefficient of 2nd order term is very small. So we conclude that they are both linearly positively related with infection rate. 
...
To detect the causal inference of our treatment variable, we need to remove the selection bias of our model by using propensity scores. Then we get our new model. 
From the output, the treatment group DevelopedCTYES is significant under level 0.001, thus we conclude that there may exist an causality between Covid-19 diagnostic rate and the developing status of the country. We also make model diagnostics, which is similar as before.
